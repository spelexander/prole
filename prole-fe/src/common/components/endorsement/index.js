...
const refToText = (ref) => <><a href={ref.url} target="_blank">Article: {truncate(ref.title)}</a><br/></>;

const Endorsement = ({loading, endorsement}) => {

    const {endorsee, type, level, references} = endorsement || {};
    ...
    return <Paper style={{display: 'flex', margin: '5px'}}>
        ...
        <div style={{marginLeft: '20px', marginTop: '5px'}}>
            <Typography style={{fontSize: '20px'}} align="left" variant="button" display="block">
                {loading ? <Skeleton width={'150px'} height={'15px'} /> : <a href={url} target="_blank">{name}</a>}
            </Typography>
